...
Serial.print("=====================================CONT MODE: "); Serial.print(modeInContinuous); Serial.print(" ! "); Serial.println(mickeymouse::getContinuousMode());

if(mickeymouse::getContinuousMode()){
  Serial.println(F("IN CONTINUOUS MODE"));

  if(cmd[i] == 'A' && !modeInContinuous){
    ++i;
    if(cmd[i] == '('){
      char myAngle[4];
      ++i;
      ...
      angle = temp.toInt();
      modeInContinuous = 1;
      pidYaw->zeroAndSetTarget(0, angle);

    }
  } 
  else if(cmd[i] == 'F' && !modeInContinuous){
    ++i;
    if(cmd[i] == '('){
      char myDist[5];
      ++i;
      ...
      distance = temp.toInt();
      modeInContinuous = 2;
      // pidL->zeroAndSetTarget(mickeymouse::wheelState.leftWheelLinPos, distance);
      // pidR->zeroAndSetTarget(mickeymouse::wheelState.rightWheelLinPos, distance);
      // pidLane->zeroAndSetTarget(0, 50);
      pidFront->zeroAndSetTarget((mickeymouse::wheelState.leftWheelLinPos + mickeymouse::wheelState.rightWheelLinPos) / 2, distance);
...
// 
  // CONTINUOUS MODE MOVEMENTS
// 
  if(mickeymouse::getContinuousMode()){
    // Turns = 1
    if(modeInContinuous == 1){
      moveController.turnLeft(pidYaw, imu);
    }
    else if(modeInContinuous == 2){ // Forwards = 2

      moveController.moveFollow(pidFront);        
